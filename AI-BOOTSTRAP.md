...
- Đọc `STATE.md` trước mọi file khác, kể cả trước `AGENTS.md`.
- Sau đó chỉ đọc mục **Đang làm / Đã thi công** trong `TASK-LEDGER.md` để không làm trùng hoặc quét nhầm dự án.
- Dòng `/master`, `- master` hoặc `• master` buộc lưu và đồng bộ tác vụ hiện tại vào Master Memory trước hành động tiếp theo.
...
Sau `STATE.md` và tác vụ đang mở trong `TASK-LEDGER.md`, đọc `AGENTS.md`, rồi đọc `AI-ROLES.md` trước khi giao việc.
