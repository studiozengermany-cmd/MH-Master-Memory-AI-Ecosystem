...
| 19 | **Unfocused image prompt** | Long prose with conflicting styles | "subject, style, mood, lighting, composition, aspect ratio"; verify current tool parameters before adding flags |
...
| # | Pattern | Bad Example | Fixed |
|---|---------|------------|-------|
| 26 | **No verification for logic task** | "which approach is better?" | "Compare both approaches, verify assumptions, then give the recommendation and concise rationale" |
| 27 | **Requesting hidden reasoning** | "show every private reasoning step" | Ask for a concise rationale, evidence, and verification instead |
...
| 36 | **Vague first turn for an agent** | "fix the auth bug" with no scope, files, or criteria | Use Template M. Front-load intent, file scope, constraints, and acceptance criteria. |
| 37 | **Context rot in long sessions** | Keeps correcting an unrelated task in the same session | Start a fresh task context or use the platform's current context-management feature; verify command names before recommending them. |
