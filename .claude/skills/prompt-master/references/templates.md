...
| [E — Reasoned Verification](#template-e--reasoned-verification) | Logic, math, analysis, debugging |
...
| [M — Agent Task Brief](#template-m--agent-task-brief) | Complex, multi-step, or agentic work |
...
## Template E — Reasoned Verification

*Use for logic-heavy tasks, math, debugging, and multi-factor analysis where the AI should verify its conclusion without exposing hidden reasoning.*

```
[Task statement]

Analyze the problem carefully and verify the result before answering.
Return:
1. Final answer
2. Concise rationale
3. Checks performed
Do not reveal private chain-of-thought or hidden scratch work.
```
...
- Creative tasks where analytical scaffolding harms the requested voice
...
*Use for Midjourney, OpenAI image tools, Stable Diffusion, Sora, Runway, and other image or video generation tools.*
...
- **Midjourney**: Use concise visual descriptors. Verify current parameter syntax before appending tool-specific flags.
- **Stable Diffusion**: Use `(word:1.3)` weight syntax. CFG scale 7 to 12. Negative prompt is mandatory.
- **OpenAI image tools**: Describe the scene and requested changes in natural language; state explicitly whether text is allowed.
...
- Image tools: use the platform's current edit/reference mode and verify tool-specific syntax when needed
...
## Template M — Agent Task Brief

*Use for complex, multi-step, or agentic tasks on Claude Code, Codex, Antigravity, or similar tools. It front-loads scope, constraints, acceptance criteria, and approval gates without depending on a particular model version.*
...
**Reasoning instruction** — add only when needed, delete otherwise:
- Hard multi-step task: `"Analyze carefully and verify the result before making changes."`
- Simple targeted change: `"This is a scoped change; keep the implementation minimal."`
...
- Compact or summarize first — use the platform's current context-management feature, after verifying the command name
```

**When to use:** Any agentic surface when the task is complex, multi-file, ambiguous, or capable of changing external state. Not needed for simple one-shot tasks.
